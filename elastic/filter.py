...
class projectFilter(Resource):
	def post(self):
		try:
			...
			query["sort"] = [{"rank.city" : {"order" : "asc", "mode" : "avg"}}]
			allBhk = ""
			...
			if category != "":
				...
				for data in allCategory:
						match.append({"range":{"score." + data:{"gte" : 1}}})
			...
			r = requests.get(esserver + '/projectnewin/data/_search?', data=json.dumps(query))
			result = json.loads(r.content)['hits']['hits']

			for key in result:
				d = key['_source']
				items.append({'name': d['name'], "key": d['key'], "style": d["general"]["style"],
					"segment": d["general"]["segment"], "rank": d["rank"]["city"], "reviews": d["review"]["count"],
					"rating": round(d["review"]["rating"],1), "score": round(d["review"]["score"],1),
				 'builder': d['builder']['name'], "propType": d['propType'],
				 'rent': d['general']['rent'], "ptype": d["general"]["ptype"], 'location': d['location'], 
