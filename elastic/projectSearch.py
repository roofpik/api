...
class projectSearchKey(Resource):
	def post(self):
		try:
...
class projectSearchReviewSummary(Resource):
	def post(self):
		try:
			...
			items = []
			esserver = 'https://search-roof-pnslfpvdk2valk5lfzveecww54.ap-south-1.es.amazonaws.com'
			r = requests.get(esserver + '/projreviewsummaryin/data/_search?', data=json.dumps(query))
