...
def imageSize(_imgType, _imgCat):
    if _imgType == 'nearby':
        return [['xs', 30, 45], ['s', 100, 150], ['m', 400, 600], ['l',
                800, 1200], ['xl', 1200, 1800]]
    if _imgType == 'residential':
      if _imgCat == 'thumbnail':
        return [['xs', 20, 24], ['s', 100, 120], ['m', 400, 480], ['l',
                800, 960], ['xl', 1200, 1440]]
...
def imageUpload():
    ...
    for key in data:
        d = data[key]
        print 'step1'
        if d['cdn'] == False and d['imgType'] == 'residential' and d['imgCat'] == 'thumbnail':
            print d
            ...
            if pathStatus['status'] == '200':
                ...
                for size in dim:
                    (imgHeight, imgWidth) = setSize(origHeight,
                            origWidth, size[1], size[2])
                    img_resize = cv2.resize(orig_img, (imgWidth,
                            imgHeight))
                    resizeImgName = d['imgName'] + '-' + size[0] \
                        + '.jpg'
                    cv2.imwrite(resizeImgName, img_resize)
                    ftp.storbinary('STOR ' + resizeImgName,
                                   open(resizeImgName, 'rb'), 1024)
        postData(key)
    ftp.quit()
